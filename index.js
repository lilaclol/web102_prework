...
function filterUnfundedOnly() {
    ...
    const listofUnfundedOnly = GAMES_JSON.filter((game) => {
...
function filterFundedOnly() {
    ...
    const listofFundedOnly = GAMES_JSON.filter((game) => {
        return game.pledged >= game.goal;
    })
...
const descriptionUnfunded = `Currently, ${unfundedGamesCount} ${unfundedGamesCount === 1 ? 'game remains': 'games remain'} unfunded. We need your help to fund these amazing games!`; 
const descriptionRaised = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length.toLocaleString('en-US')} games. `;
const descriptionText= descriptionRaised + descriptionUnfunded;
